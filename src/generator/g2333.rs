...
impl Generator for G2333 {
    ...
    fn regenerate(&mut self) {
        ...
        while self.test_cases.len() < self.count {
            ...
            for _ in 0..TEST_SIZE {
                ...
                answer += (random as u128).pow(2);
            }
            debug!("answer ^ 2: {}", answer);
            self.answer.push((answer as f64).sqrt() as u128);
    ...
    fn check_answer(&self, data: &str) -> Result<bool, Box<dyn std::error::Error>> {
        let result: Vec<u128> = serde_json::from_str(data).expect("JSON was not well-formatted");
